...
func RunBulk(ctx context.Context, svc *service.SegmentationService, logger *log.Logger, bulksize uint64) error {
	...
	for i := 0; i < workers; i++ {
		...
		go func(workerID int) {
			...
			for r := range ch {
				...
				segmentations := make([]models.Segmentation, len(r))
				for i, rec := range r {
					segmentations[i] = models.Segmentation{
						UserID:           rec.userID,
				...
				result, errors := svc.BulkCreate(ctx, &segmentations)
				if len(errors) > 0 {
	...
	for {
		...
		for i := uint64(0); i < bulksize; i++ {
			...
			records = append(records, chRecord)
		}
		if len(records) > 0 {
			select {
			case <-ctx.Done():
